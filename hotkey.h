...
public:
    ...
    ~Hotkey()
    {
        for (std::size_t i = 0; i < _M_func_list.size(); ++i)
        {
            ::UnregisterHotKey(NULL, static_cast<int>(i));
        }
    }
    
    bool registerShortcut(const QKeySequence& shortcut, const std::function<void(::MSG*)>& func)
    {
        int key = shortcut[0];
        
        Qt::Key keyCode = Qt::Key(key & ~Qt::KeyboardModifierMask);
        Qt::KeyboardModifiers modifiers = Qt::KeyboardModifiers(key & Qt::KeyboardModifierMask);
        
        ::BOOL ok = 
            ::RegisterHotKey(NULL,
                         _M_func_list.size(),
                         nativeModifiers(modifiers) + MOD_NOREPEAT,
                         nativeKeycode(keyCode));

        if (ok) _M_func_list.push_back(func);
    ...
    virtual bool nativeEventFilter(const QByteArray& eventType, void* message, long* result) override
    {
        ...
        {
            _M_func_list[msg->wParam](msg);
        }
        return false;
    }
    
    quint32 nativeKeycode(Qt::Key keycode, bool* ok = nullptr)
    {
        if (ok) *ok = true;
...
private:
    std::vector<std::function<void(::MSG*)>> _M_func_list;
};
